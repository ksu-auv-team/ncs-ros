...
def img_callback(data):
    ...
    # cam = cv2.VideoCapture('http://' + sys.argv[1] + ':8080')
    # print('http://' + sys.argv[1] + ':8080/?action=stream')
    # if not cam.isOpened():
    #     print('error: camera not opened')
        # rospy.sleep(.5)
        # cam = cv2.VideoCapture('udpsrc port=5000 ! gdpdepay ! rtph264depay ! avdec_h264 ! videoconvert ! autovideosink sync=false')
...
listener = rospy.Subscriber('inception_input', Image, img_callback)    
rospy.spin()
